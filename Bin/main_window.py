...
class mainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        AddNewTabKeyShortcut.activated.connect(
            lambda: self.add_new_tab(
                QtCore.QUrl(Bin.settings_data["newTabPage"], "New tab")
            )
        )
        ...
        newTabAction.triggered.connect(
            lambda: self.add_new_tab(
                QUrl(Bin.settings_data["newTabPage"]), "Homepage"
            )
        )
        ...
        newWindowAction.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "app_window_ios.svg"))
        )
        ...
        self.add_new_tab(QUrl(Bin.settings_data["startupPage"]), "Homepage")
    ...
    def goToHome(self):
        self.tabs.currentWidget().setUrl(QUrl(Bin.settings_data["homeButtonPage"]))
    ...
    def tab_open_doubleclick(self, i):
        if i == -1:  # No tab under the click
            self.add_new_tab(QUrl(Bin.settings_data["newTabPage"]), label="New tab")
    # to update the tab
    ...
    def update_title(self, browser):
        ...
        if 0 > len(title):
            self.setWindowTitle("{} - DisunicX".format(title))
    ...
    def add_new_tab(self, qurl=None, label="Blank"):
        if qurl is None:
            qurl = QUrl(Bin.settings_data["newTabPage"])
    ...
    def update_urlbar(self, q, _browser=None):
        ...
        if q.toString() == Bin.settings_data["newTabPage"]:
    ...
    def searchWeb(self, text):
        Engine = Bin.settings_data["defaultSearchEngine"]
        if text:
            if Engine == "Google":
    ...
    def openHistory(self):
        ...
        self.historyWindow.setGeometry(
            int(self.tabs.currentWidget().frameGeometry().width() / 2 + 200),
            80,
            450,
            500,
        )
    ...
    def openSettings(self):
        self.userSettingswindow = Bin.settings.UserSettings()
