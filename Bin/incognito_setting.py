...
class UserSettings(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.settings_data = Bin.init.settings_data
        self.default_search_engine = self.settings_data["defaultSearchEnginein"]
    ...
    def init_ui(self):
        ...
        self.startup_page.setText(self.settings_data["startupPagein"])
        ...
        self.home_button_page.setText(self.settings_data["homeButtonPagein"])
        ...
        self.new_tab_page.setText(self.settings_data["newTabPagein"])
        ...
        self.save_settings.clicked.connect(self.closeWindow)
...
QComboBox::down-arrow{
    image: url(./VisualElements/arrow-down-12.png);
    border : none;
}
QComboBox::drop-down{
    border: none;
...
QLabel#label_4, QLabel#label_5, QLabel#label_6, QLabel#label_8{
    ...
    def saveChangesToJson(self):  # startup pg
        if len(self.startup_page.text()) > 0:
            self.settings_data["startupPagein"] = self.startup_page.text()
            with open("Incognito_Settings.json", "w") as f:
                json.dump(self.settings_data, f, indent=2)

        if len(self.home_button_page.text()) > 0:
            self.settings_data["homeButtonPagein"] = self.home_button_page.text()
            with open("Incognito_Settings.json", "w") as f:
                json.dump(self.settings_data, f, indent=2)

        if len(self.new_tab_page.text()) > 0:
            self.settings_data["newTabPagein"] = self.new_tab_page.text()
            with open("Incognito_Settings.json", "w") as f:
                json.dump(self.settings_data, f, indent=2)

    def addDropDownItemToJson(self):
        self.settings_data[
            "defaultSearchEnginein"
    ...
    def retranslateUi(self):
        ...
        self.setWindowTitle("DisunicX Incognito Setting")
