...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.tabs = Bin.widgets.Tabs()  # create tabs
        ...
        AddNewTabKeyShortcut.activated.connect(
            lambda: self.add_new_tab(
                QtCore.QUrl(Bin.init.settings_data["newTabPagein"], "New tab")
            )
        )
        ...
        back_btn.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "left-arrow.svg"))
        )
        ...
        forward_butn.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "right-arrow.svg"))
        )
        ...
        self.reload_butn.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "refresh.svg"))
        )
        ...
        self.stop_btn.setIcon(QIcon(os.path.join("VisualElements", "icons", "cross.svg")))
        ...
        self.home_button.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "home.svg"))
        )
        self.home_button.clicked.connect(self.goToHome)
        self.navbar.addWidget(self.home_button)

        # Add Address bar
        self.url_bar = Bin.widgets.AddressBar()
        ...
        self.httpsicon = Bin.widgets.SSLIcon()
        ...
        ContextMenuButton.setIcon(
            QIcon(os.path.join("VisualElements", "icons", "more.svg"))
        )  # Add icon
        ...
        newTabAction.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "newtab.svg"))
        )
        newTabAction.triggered.connect(
            lambda: self.add_new_tab(
                QUrl(Bin.init.settings_data["newTabPagein"]), "Homepage"
            )
        )
        ...
        # New window action
        newWindowAction = QAction("New Incognito", self)
        newWindowAction.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "incognito.svg"))
        )
        ...
        CloseTabAction.setIcon(
            QIcon(os.path.join("VisualElements", "icons", "closetab.svg"))
        )
        ...
        # IncognitoAction = QAction("Incognito", self)
        # IncognitoAction.setIcon(QIcon(os.path.join("VisualElements", "icons", "incognito.svg")))
        # IncognitoAction.triggered.connect(self.incognito)
        # context_menu.addAction(IncognitoAction)
        ...
        CopySiteAddress = QAction(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "url.svg")),
            "Copy site url",
            self,
        )
        ...
        PasteAndGo = QAction(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "paste.svg")),
            "Paste and go",
            self,
        )
        ...
        # ViewHistory.setIcon(QIcon(os.path.join("VisualElements", "icons", "history.svg")))
        ...
        OpenPgAction.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "openclickhtml.svg"))
        )
        ...
        SavePageAs.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "save-disk.svg"))
        )
        ...
        PrintThisPageAction.setIcon(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "printer.svg"))
        )
        ...
        PrintPageWithPreview = QAction(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "printerprev.svg")),
        ...
        SavePageAsPDF = QAction(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "adobepdf.svg")),
            "Save as PDF",
            self,
        )
        ...
        userSettingsAction = QAction(
            QtGui.QIcon(os.path.join("VisualElements", "icons", "settings.svg")),
            "Settings",
            self,
        )
        ...
        HelpMenu.setIcon(QIcon(os.path.join("VisualElements", "icons", "question.svg")))

        # About action
        AboutAction = QAction("About this browser", self)
        AboutAction.setIcon(QIcon(os.path.join("VisualElements", "icons", "info.svg")))
        ...
        self.add_new_tab(QUrl(Bin.init.settings_data["startupPagein"]), "Homepage")
    ...
    def goToHome(self):
        self.tabs.currentWidget().setUrl(QUrl(Bin.init.settings_data["homeButtonPagein"]))
    ...
    def visitGithub(self):
        self.add_new_tab(
            QUrl("https://github.com/DisunicCorporation/DisunicX-Browser"), "Github"
        )
    ...
    def open_local_file(self):
        ...
        if filename:
            try:
                ...
                dlg = Bin.errors.fileErrorDialog()
                dlg.exec_()
    ...
    def print_this_page(self):
        try:
            handler_print = Bin.printer.PrintHandler()
    ...
    def PrintWithPreview(self):
        handler = Bin.printer.PrintHandler()
    ...
    def tab_open_doubleclick(self, i):
        if i == -1:  # No tab under the click
            self.add_new_tab(QUrl(Bin.init.settings_data["newTabPagein"]), label="New tab")
    # to update the tab
    ...
    def update_title(self, browser):
        ...
        if 0 > len(title):
            self.setWindowTitle("{} - DisunicX Incognito".format(title))

        else:
            self.setWindowTitle("DisunicX Incognito")

    # function to add new tab
    def add_new_tab(self, qurl=None, label="Blank"):
        if qurl is None:
            qurl = QUrl(Bin.init.settings_data["newTabPagein"])

        _browser = QWebEngineView()  # Define the main webview to browser the internet

        # Set page
        _browser.setPage(Bin.widgets.customWebEnginePage(_browser))
    ...
    def showErrorDlg(self):
        dlg = Bin.errors.errorMsg()
        dlg.exec_()
    def about(self):
        self.AboutDialogue = Bin.about.AboutDialog()
        self.AboutDialogue.show()

    # def incognito(self):
    #     self.IncogDialogue = Bin.incognito.MainWindow()
    #     self.IncogDialogue.show()
    ...
    def update_urlbar(self, q, _browser=None):
        ...
        if q.toString() == Bin.init.settings_data["newTabPagein"]:
            self.httpsicon.setPixmap(
                QPixmap(os.path.join("VisualElements", "icons", "info_24.svg"))
            )
            self.httpsicon.setToolTip("New Tab\n\nDisunic://newtab")
            self.url_bar.clear()
            ...
            if q.scheme() == "https":
                # secure padlock icon
                self.httpsicon.setPixmap(
                    QPixmap(os.path.join("VisualElements", "icons", "security.svg"))
                )
                self.httpsicon.setToolTip(
                    "Connection is secure\n\nYour information is privet when it is sent to this site."
                )

            elif q.scheme() == "file":
                self.httpsicon.setPixmap(
                    QPixmap(os.path.join("VisualElements", "icons", "info_24.png"))
                )
                ...
                self.httpsicon.setPixmap(
                    QPixmap(os.path.join("VisualElements", "icons", "warning.png"))
                )
                self.httpsicon.setToolTip("Connection not secure")
    ...
    def searchWeb(self, text):
        Engine = Bin.init.settings_data["defaultSearchEnginein"]
        if text:
            if Engine == "Google":
    ...
    def updateHistory(self):
        ...
        data = Bin.cursor.execute("SELECT * FROM history")
        siteInfoList = data.fetchall()

        for i in range(len(siteInfoList)):
            if url == siteInfoList[i][2]:
                Bin.cursor.execute("DELETE FROM history WHERE url = ?", [url])
        ...
        Bin.connection.commit()

    # def openHistory(self):
    #     self.historyWindow = Bin.history.HistoryWindow()
    ...
    def openSettings(self):
        self.userSettingswindow = Bin.incognito_setting.UserSettings()
